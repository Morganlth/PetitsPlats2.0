...
// #\_CONSTRUCTOR_\
    ...
    // __THIS
    constructor () { this.#recipe_set(...arguments) }
...
// #\_FUNCTIONS_\
    ...
    #recipe_set()
    {
        ...
        for (const VALUE of [this.#recipe_NAME, this.#recipe_DESCRIPTION, this.#recipe_INGREDIENTS.map(({ingredient}) => ingredient), this.#recipe_APPLIANCE, this.#recipe_USTENSILS]) this.#recipe_setKeywords(VALUE)
    }

    #recipe_setVars({name, description, appliance, ingredients, ustensils})
    {
    ...
    #recipe_setKeywords(value) { Recipe.__recipe_TREE.tree_addWords(str_compressed(value).split(' '), this) }


}

const CONFIG_WORD_MIN_LENGTH = 3

const STR_COMPRESSED_REGEX = new RegExp(`[^\\w\\s]|\\b\\w{1,${CONFIG_WORD_MIN_LENGTH - 1}}\\b`, 'gm')

function str_normalize(s = '') { return s?.normalize('NFD').replace(/[\u0300-\u036f]/g, '') }

function str_compressed(value)
{
